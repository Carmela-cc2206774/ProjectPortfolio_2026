...
class _CartScreenState extends ConsumerState<CartScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final products = ref.read(productProviderNotifier.notifier).products;
    ;
                    ...
                    Column(
                      children: List.generate(cart.length, (index) {
                        Product product = products.firstWhere((product) =>
                            product.id == cart.elementAt(index).id &&
                            cart.elementAt(index).quantity != 0);
                        return Padding(
                          ...
                          child: FutureBuilder<int>(
                              ...
                              builder: (context, cartProvider) {
                                final quantity = cartProvider.data ?? 9;
  ...
  Widget myBottomSheet() {
    final cartState = ref.watch(cartProviderNotifier);
    final isCartEmpty = ref.read(cartProviderNotifier.notifier).isCartEmpty();
    return Container(
      height: 220,
